...
HI_S32 SAMPLE_MISC_WbcDump(VO_DEV VoDev, HI_U32 u32Cnt)
{
    ...
    printf("usage: ./vou_wbc_dump [vodev] [frmcnt]. sample: ./vou_wbc_dump 0 5\n\n");

    HI_MPI_VO_SetWbcDepth(VoDev, 1);

    /* Get Frame to make file name*/
    s32Ret = HI_MPI_VO_WbcGetScreenFrame(VoDev, &stFrame);
    ...
    HI_MPI_VO_WbcReleaseScreenFrame(VoDev, &stFrame);
    ...
    for (i=0; i<u32Cnt; i++)
    {
        ...
        s32Ret = HI_MPI_VO_WbcReleaseScreenFrame(VoDev, &stFrame);
        if (HI_SUCCESS != s32Ret)
        {
