...
******************************************************************************/
// Easy_Handle g_Easy_H;

// void *LOTO_COMM_AUDIO_AencProc_(void *parg)
// {
//     HI_S32 s32Ret;
//     HI_S32 AencFd[2];
//     AENC_CHN AencChn;
...
//     if (aacEncoder_SetParam(g_Enc_H, AACENC_BITRATE, 24000) != AACENC_OK)
//     {
//         aacEncClose(g_Enc_H);
...
// //     faacEncHandle hEncoder = NULL;		//aac句柄
...
// //     pConfiguration->aacObjectType=MAIN;
...
//                 // nMaxOutputBytes = stStream.u32Len;
//                 // int bAACBufferSize = nMaxOutputBytes;//提供足够大的缓冲区
//                 // unsigned char *pbAACBuffer = (unsigned char*)malloc(bAACBufferSize * sizeof(unsigned char));  
//                 // unsigned int out_len = 0;

//                 // printf("%s: stStream.u32Len = %d, bAACBufferSize = %d \n", __FUNCTION__, stStream.u32Len, nMaxOutputBytes);
//                 // printf("HI_MPI_VENC_GetStream: i = %d, timestamp = %"PRIu64" \n", i, stStream.u64TimeStamp/1000);

//                 // if((Easy_AACEncoder_Encode(g_Easy_H, stStream.pStream, stStream.u32Len, pbAACBuffer, &out_len)) > 0)
//                 // {
//                 //     printf("%s: aenc stream Write file, i = %d, out_len = %d \n", __FUNCTION__, i, out_len);
//                 //     /* save audio stream to file */
//                 //     // fwrite(stStream.pStream,1,stStream.u32Len, pstAencCtl->pfd);
//                 //     fwrite(pbAACBuffer, 1, out_len, pfd[i]);
//                 // }

//                 // if (hEncoder != NULL)
//                 // {
//                 //     printf("%s:  before faacEncEncode \n", __FUNCTION__);
//                 //     out_len = faacEncEncode(hEncoder, (int*) stStream.pStream, stStream.u32Len, pbAACBuffer, bAACBufferSize);
//                 //     printf("%s: aenc stream Write file, i = %d, out_len = %d \n", __FUNCTION__, i, out_len);
//                 //     if (out_len > 0)
//                 //     {
//                 //         fwrite(pbAACBuffer, 1, out_len, pfd[i]);
...
//                     {
//                         fwrite(outbuf, 1, out_args.numOutBytes, pfd[i]);
...
//                 // {
//                 //     printf("%s: aenc stream bSendAdChn \n", __FUNCTION__);

//                 //     HI_MPI_ADEC_SendStream(pstAencCtl->AdChn, &stStream, HI_TRUE);
//                 // }

//                 // printf("%s: aenc stream Write file \n", __FUNCTION__);
//                 /* save audio stream to file */
//                 // fwrite(stStream.pStream,1,stStream.u32Len, pstAencCtl->pfd);
...
void *SAMPLE_COMM_AUDIO_AencProc(void *parg)
{
    ...
    /* create file for save stream*/
    sprintf(aszFileName, "audio_chn%d.aac", pstAencCtl->AeChn);
    ...
    while (pstAencCtl->bStart)
    {
        ...
        if (FD_ISSET(AencFd, &read_fds))
        {
            ...
            int bAACBufferSize = stStream.u32Len;//提供足够大的缓冲区
            unsigned char *pbAACBuffer = (unsigned char*)malloc(bAACBufferSize * sizeof(unsigned char));  
            unsigned int out_len = 0;

            printf("%s: stStream.u32Len = %d \n", __FUNCTION__, stStream.u32Len);
            printf("HI_MPI_VENC_GetStream:  timestamp = %"PRIu64" \n", stStream.u64TimeStamp/1000);

            // if((Easy_AACEncoder_Encode(g_Easy_H, stStream.pStream, stStream.u32Len, pbAACBuffer, &out_len)) > 0)
            // {
            //     // printf("%s: aenc stream Write file, out_len = %d \n", __FUNCTION__, out_len);
            //     /* save audio stream to file */
            //     // fwrite(stStream.pStream,1,stStream.u32Len, pstAencCtl->pfd);
            //     fwrite(pbAACBuffer, 1, out_len, pfd);
            // }
            free(pbAACBuffer);
            ...
            //         printf("%s: aenc stream length: %d, aac length = %d \n", __FUNCTION__, stStream.u32Len, out_args.numOutBytes);
            //     }
            //     else
            //     {
            //         printf("%s: Encoding 0 bytes\n", __FUNCTION__);
...
//     pthread_create(&pstAenc->stAencPid, 0, LOTO_COMM_AUDIO_AencProc_, pstAenc);
        ...
        AIO_ATTR_S *pstAioAttr, HI_BOOL bAnrEn, AUDIO_RESAMPLE_ATTR_S *pstAiReSmpAttr)
{
    ...
    {
        if (s32Ret = HI_MPI_AI_EnableChn(AiDevId, i))
        {
        ...
        if (HI_TRUE == bAnrEn)
        {
            if (s32Ret = HI_MPI_AI_EnableAnr(AiDevId, i))
            {
        ...
        {
            if (s32Ret = HI_MPI_AI_EnableReSmp(AiDevId, i, pstAiReSmpAttr))
            {
