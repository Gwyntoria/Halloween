            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    // SAMPLE_PRT("[%s] HI_MPI_VENC_GetStream:  timestamp = %"PRIu64" \n", log_Time(), stStream.pstPack[0].u64PTS/1000);
                    if (HI_SUCCESS != s32Ret)
                    {
                        //free(stStream.pstPack);
                        stStream.pstPack = NULL;
...
HI_S32 SAMPLE_COMM_VENC_StartGetStream(HI_S32 s32Cnt)
{
    ...
    HI_S32 nRet = pthread_create(&gs_VencPid, 0, SAMPLE_COMM_VENC_GetVencStreamProc, (HI_VOID*)&gs_stPara);
