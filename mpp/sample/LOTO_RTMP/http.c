...
#define CONTENT_TYPE_JSON_STR "Content-Type: application/json;charset=utf-8\r\n"
#define CONTENT_TYPE_TEXT_STR "Content-Type: text/plain;charset=utf-8\r\n"
...
static void http_ssl_free(ft_http_client_t* http)
{
    if(http->ssl != NULL)
    {
        SSL_shutdown_0(http->ssl);
        SSL_free_0(http->ssl);
        http->ssl = NULL;
    }
    if(http->ctx != NULL)
    {
        SSL_CTX_free_0(http->ctx); 
        http->ctx = NULL;
    }
}
...
HTTP_API void ft_http_destroy(ft_http_client_t* http)
{
    ...
    free_member(http->date);
    free_member(http->content_type);
...
HTTP_API int ft_http_get_body_len(ft_http_client_t* http)
{
    if(http == NULL)
    {
        return 0;
...
HTTP_API char* ft_http_get_header_date(ft_http_client_t* http)
{
    if(http == NULL)
    {
        return NULL;
...
HTTP_API char* ft_http_get_content_type(ft_http_client_t* http)
{
    if(http == NULL)
    {
        return NULL;
...
#ifdef WIN32
static const unsigned __int64 epoch = ((unsigned __int64) 116444736000000000UL);
...
static int parser_field_value(ft_http_client_t* http)
{
        ...
        http->header_field && http->header_value)
    {
        ...
        else if(_stricmp(http->header_field, "Date") == 0)
        {
            free_member(http->date);
            http->date = _strdup(http->header_value);
        }
        else if(_stricmp(http->header_field, "Content-Type") == 0)
        {
            free_member(http->content_type);
            http->content_type = _strdup(http->header_value);
        }
        else
        {
...
static int http_check_error(ft_http_client_t* http, int mode, int ret)
{
    ...
    {
        int error =  SSL_get_error_0(http->ssl, ret);
...
static int http_read_write(ft_http_client_t* http, const char* data, int len, int read)
{
    ...
    do
    {
        ...
        {
#if FT_SUPPORT_HTTPS
            r = read ? SSL_read_0(http->ssl, (char*)data + n, len - n) : SSL_write_0(http->ssl, data + n, len - n);
#endif
        }
        else
        {
...
static int http_ssl_connect(ft_http_client_t* http)
{
    ...
    http->ctx = SSL_CTX_new_0(SSLv23_client_method_0());
    if(http->ctx == NULL)
    {
        return -1;
    }
    http->ssl = SSL_new_0(http->ctx);
    if(http->ssl == NULL)
    {
        return -1;
    }
    if(SSL_set_fd_0(http->ssl, http->fd) == 0)
    {
        return -1;
    }
    ...
    do
    {
        ssl_ret = SSL_connect_0(http->ssl);
...
static int http_internal_sync_request(ft_http_client_t* http, const char* url, const char* post_data, int post_data_len, const char* user_header, int user_header_len)
{
    // 定义变量和初始化
    ...
    int r = 0;
    int parsed = 0;
    unsigned short port = 443;  // 默认使用443端口（HTTPS）
    ...
    if (r != ERR_OK)
    {
        printf("http_connect_host err with %#x\n", r);
        return http->error_code = r;
    }

    // 写入请求行
    #define CHECK(nwrite) \
        if ((nwrite) <= 0) { return http->error_code = ERR_SOCKET_WRITE; }
    ...
    // 写入POST请求的 Content-Type 和 Content-Length
                                              ...
                                              const char* user_header, int user_header_len)
{
    ...
    http->error_code = http_internal_sync_request(http, url, post_data, post_data_len, user_header, user_header_len);
