...
HI_S32 SAMPLE_AUDIO_AiAenc(AIO_ATTR_S *pstAioAttr)
{
    ...
    FILE        *pfd0 = NULL, *pfd1 = NULL;
    

    AACENC_ERROR    aacenc_err;
    ...
    ********************************************/
    if (HI_TRUE == bSendAdec)
    {
        ...
        // AeChn = 1;
        // pfd0 = SAMPLE_AUDIO_OpenAencFile(1, gs_enPayloadType);
        // if (!pfd0)
        // {
        //     SAMPLE_DBG(HI_FAILURE);
        //     return HI_FAILURE;
        ...
        s32Ret = SAMPLE_COMM_AUDIO_CreatTrdAencAdec(AeChn, AdChn, pfd0);
        if (s32Ret != HI_SUCCESS)
        {
            SAMPLE_DBG(s32Ret);
            return HI_FAILURE;
