...
typedef struct AudioSpecificConfig {
    uint8_t audio_object_type;      /* audio type (profile) */
...
int rtmp_sender_write_aac_frame(void* handle, uint8_t* data, int size,
                                uint64_t dts_us, uint32_t start_time)
{
    ...
    while (1) {
        ...
        if (rtmp_xiecc->audio_config_ok == 0) {
            ...
            output[offset++] =
                gen_audio_tag_header(rtmp_xiecc->config); // sound format aac
            output[offset++] = 0x00;                      // aac sequence header

            // flv AudioTagBody --AudioSpecificConfig
            uint8_t audio_object_type =
                rtmp_xiecc->config.audio_object_type + 1;
            output[offset++] = (audio_object_type << 3) |
                               (rtmp_xiecc->config.sample_frequency_index >> 1);
            output[offset++] =
                ((rtmp_xiecc->config.sample_frequency_index & 0x01) << 7) |
...
int rtmp_sender_write_avc_frame(void* handle, uint8_t* data, int size,
                                uint64_t dts_us, int key)
{
    ...
    while (1) {
        ...
        if ((nal[0] & 0x1f) == 0x07) {
            ...
            output[offset++] =
                (uint8_t)(nal_len >>
                          8); // sequence parameter set length high 8 bits
            output[offset++] =
            ...
            output[offset++] = (uint8_t)(nal_len_pps); // picture parameter set
                                                       // length low 8 bits
